...
const Shedule: React.FC<PropsType> = () => {
    ...
    useEffect(() => {
        fetch(`api/doctors/byAreaAndSpec?areaId=${areaId}&specId=${specId}`, {method: 'GET'})
        .then(response => response.json())
    ...
    return(
        ...
        <table>
            ...
            {shedules.map((s: SheduleObj, k) => (
                <tr>
                    <td><Label key="label">{s.day.name}</Label></td>
                    <td><TimePicker key="beginTime" inputReadOnly={true} defaultValue={dayjs(s.beginTime, "HH:mm:ss")} minuteStep={30} format="HH:mm" onChange={(e) => s.beginTime = dayjs(e, "HH:mm:ss").format("HH:mm:ss")}/></td>
                    <td><TimePicker key="beginTime" inputReadOnly={true} defaultValue={dayjs(s.endTime, "HH:mm:ss")} minuteStep={30} format="HH:mm" onChange={(e) => s.endTime = dayjs(e, "HH:mm:ss").format("HH:mm:ss")}/></td>
                </tr>
            ))}
            </tbody>
        </table><br/>
        <Button onClick={() => handleSubmitSheduleBtn()} type="primary" disabled={doctorId ? false : true}>Сохранить</Button>
    </React.Fragment>
    )
}
